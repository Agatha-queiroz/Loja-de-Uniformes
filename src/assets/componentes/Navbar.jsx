...
function Navbar({cartContagem}) {
  return (
    <nav className="navbar">
      <img  className="logo" src={Logo} alt="Logo da Empresa"/>
      
      <div className="pesquisar-box">
      <input id="barra-pesq" type="text" placeholder="Pesquisar"/>{ /* isso vai funcionar :) onClick={()=> {props.openCart(true)}}*/ } 
      <button  className="btn-pesq"><img src={Procurar} alt="" /></button>
      </div>

      <div className="navbar-cart"> Carrinho <img src={compras} alt="carrinho" /><span className="Cont-carr"> {cartContagem}</span></div>
    </nav>
  );
}
